...
-- [[ IWE Notes Fuzzy Find & Backlinks ]]
local notes_dir = vim.fn.expand '~/notes'
vim.keymap.set('n', '<leader>sf', function()
  require('mini.pick').builtin.files({}, { source = { cwd = notes_dir, name = 'Notes Files' } })
end, { desc = '[S]earch Notes [F]iles (fuzzy)' })
vim.keymap.set('n', '<leader>sg', function()
  require('mini.pick').builtin.grep_live({}, { source = { cwd = notes_dir, name = 'Notes Grep' } })
end, { desc = '[S]earch Notes [G]rep (live)' })
vim.keymap.set('n', '<leader>sb', function()
  require('iwe.picker').backlinks()
end, { desc = '[S]earch Notes [B]acklinks' })
vim.keymap.set('n', '<leader>sr', function()
  require('iwe.picker').blockreferences()
end, { desc = '[S]earch Notes [R]eferences' })
vim.keymap.set('n', '<leader>so', function()
  require('iwe.picker').headers()
end, { desc = '[S]earch Notes [O]utline (headers)' })
...
-- [[ Custom LSP Keymaps (separate from kickstart's LspAttach) ]]
...
end, { desc = '[S]ave [S]ession (auto-named from CWD)' })

vim.keymap.set('n', '<leader>sl', function()
  local session_dir = vim.fn.stdpath('data') .. '/sessions'
  local files = vim.fn.readdir(session_dir) or {}
  ...
  vim.ui.select(names, { prompt = 'Load session:' }, function(choice)
    if choice then
      require('mini.sessions').read(choice)
      vim.notify('Session loaded: ' .. choice)
    end
  end)
end, { desc = '[S]ession [L]ist (load via ui.select)' })
